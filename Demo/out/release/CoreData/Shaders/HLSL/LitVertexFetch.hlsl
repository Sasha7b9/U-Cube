...
uniform float cClipRange;
#endif

#ifdef COMPILEPS
Texture2D tRampMap : register(t0);
SamplerState sRampMap : register(s0);
#endif

#ifdef COMPILEVS
Texture2D tDisplaceMap : register(t1);
SamplerState sDisplaceMap : register(s1);
#endif
...
void VS(float4 iPos : POSITION,
	...
	out float4 oDisplaceTexel : TEXCOORD8,
	out float4 oChannelFactorDisp: TEXCOORD9,
    #ifdef PERPIXEL
        #ifdef SHADOW
...
void PS(
	...
	float4 iDisplaceTexel : TEXCOORD8,
	float4 iChannelFactorDisp : TEXCOORD9,
    #ifdef PERPIXEL
        #ifdef SHADOW
    ...
    out float4 oColor : OUTCOLOR0)
{
	...
	float d = float(iDisplaceTexel.r * iChannelFactorDisp.r + iDisplaceTexel.g * iChannelFactorDisp.g + iDisplaceTexel.b * iChannelFactorDisp.b) * (cRange.y - cRange.x) + cRange.x;
	...
	// Get material diffuse albedo
		...
		float4 diffInput = Sample2D(RampMap, float2(d, 0.5));
